...
import { CalendarIcon, UserIcon, EditIcon, ArrowLeftIcon, ArchiveIcon } from 'lucide-react';
...
// Helper to format date for display (e.g., "Monday, January 2, 2025")
...
function parseDateParam(param: string): { year: number; month: number; day: number; isValid: boolean } {
    try {
        ...
        // Case 1: DD-MM-YYYY (e.g., 02-01-2025 -> 2nd Jan 2025)
        // Regex matches: start, 1-2 digits, hyphen, 1-2 digits, hyphen, 4 digits, end
        const ddMmYyyyRegex = /^(\d{1,2})-(\d{1,2})-(\d{4})$/;
        const ddMmYyyyMatch = param.match(ddMmYyyyRegex);
        ...
        // Case 2: YYYY-MM-DD (e.g., 2025-01-02 -> 2nd Jan 2025)
        ...
        // Case 3: Month-Day-Year (e.g., january-02-2025 or january-2-2025)
        const monthNames = [
        ...
        if (parts.length === 3) {
            ...
            const potentialYear = parseInt(parts[2], 10); // Assume year is last format per user request examples
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    // Basic metadata, could be enhanced with fetched data if creating dynamic metadata
    return {
        title: `NYT Crossword Answer`,
        description: `Solution for the New York Times crossword puzzle.`,
    };
}
...
export default async function CrosswordAnswerPage({ params }: PageProps) {
    ...
    if (!parsed.isValid) {
        // If we can't parse it, maybe it is just garbage, or we could redirect to archive
        // For now, let's show 404 behavior or redirect to archive
        // But maybe check if it's "today" or something? User request didn't specify invalid handling details.
        // Let's rely on standard 404 if data fetch fails, but for format...
        // User specific strict redirects.
        return (
    ...
    // If current param doesn't match canonical (e.g. was 02-01-2025 or january-2-2025), redirect
            ...
            <div className="container mx-auto px-4">
                <div className="mx-auto max-w-5xl">
                    ...
                    <div className="mb-8 rounded-xl bg-white p-6 shadow-xl">
                        <div className="text-center">
                            ...
                            <h1 className="mt-3 text-3xl font-bold text-gray-900 md:text-4xl">
                                {puzzle.title || 'Crossword Solution'}
                            </h1>
                    ...
                    <div className="mb-8 flex justify-center">
                        <Link
                            href={`/play-crossword/${year}/${month.toString().padStart(2, '0')}/${paddedDay}`}
