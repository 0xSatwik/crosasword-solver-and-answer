...
export default function NytArchivePage() {
  ...
  const handleDateSelect = async (day: number) => {
    ...
    const formattedDate = formatDateForApi(year, month, day);
    ...
    try {
      ...
      if (result?.success && result?.data) {
        ...
        setError(`No puzzle data available for ${formatDisplayDate(formattedDate.replace(/-/g, '/'))}`);
        setPuzzleData(null);
      }
    } catch (err) {
                    ...
                    {day && (
                      isFutureDate(year, month, day) ? (
                        ...
                        <button
                          ...
                          className={`inline-block h-8 w-8 rounded-full py-1 transition-colors hover:bg-blue-100 hover:text-blue-700 ${
          ...
          {puzzleData && !isLoading && (
            <div className="mt-8">
              ...
              <div className="mb-8 flex justify-center">
                <Link
                  href={`/nyt-crosswords/${selectedDate?.split('-')[0]}/${selectedDate?.split('-')[1]}/${selectedDate?.split('-')[2]}`}
